// ================================================================
//  A W A K E N I N G  —  gesture.js  (v8 + GRS hooks)
//
//  ── UNCHANGED from v8 ──────────────────────────────────────────
//   . Zero core changes.
//
//  ── ONLY ADDITIONS (2 lines in draw()) ─────────────────────────
//  1. After drawPaper():
//       if(window.GRS) GRS.renderBG();
//     Lets gestureReactions.js draw reactive visuals BEHIND character.
//
//  2. After drawHUD():
//       if(window.GRS) GRS.update();
//     Lets gestureReactions.js tick its signals, particles + audio.
//
//  Both lines are safe no-ops when gestureReactions.js is absent.
//  gestureReactions.js reads globals from this file (read-only).
//  No tracking is duplicated. No existing logic is changed.
// ================================================================
...
// ─────────────────────────────────────────────────────────────────
function preload(){
  // ── Images only — these never block indefinitely ────────────────
  ...
  // ── Audio: loaded lazily via Web Audio after setup() ────────────
  // loadSound() is NOT called here — it blocks p5 forever on 404.
  // _loadAudioLazy() is called from setup() instead.
}

// Lazy audio loader — called from setup(), never from preload()
// Uses fetch() which fails silently on 404 instead of hanging forever.
let _audioCtx = null;
function _loadAudioLazy(){
  ...
  function fetchSound(file, onBuf){
    fetch(file)
      ...
      .then(buf=>onBuf(buf))
      .catch(()=>{}); // silent — missing file = no sound, app still works
  }

  // Whistle
  fetchSound("whistle.mp3", buf=>{
    // Wrap buffer in a play-compatible object matching p5.sound interface
    snd = {
      _buf: buf,
...
function setup(){
  ...
  textFont("sans-serif");
  buildPaper();
  _loadAudioLazy(); // load whistle/wobble sounds without blocking preload
}

// ─────────────────────────────────────────────────────────────────
//  DRAW  ← only changes in this function: 2 GRS hook lines added
// ─────────────────────────────────────────────────────────────────
function draw(){
  if(!blobsReady){
    blobsReady=true;
  ...
  // ── Gesture Reaction System: background visuals (behind character) ─
  ...
  // ── Gesture Reaction System: detect gestures + tick audio ─────────
  ...
  // ── Gesture Reaction System: front layer (confetti in front of character) ─
...
class BezierArm{
  ...
  update(r,bx,by){
    ...
    this.smElbX = lerp(this.smElbX, elbowTarget.x, 0.15);
    this.smElbY = lerp(this.smElbY, elbowTarget.y, 0.15);
    this.smWriX = lerp(this.smWriX, wristTarget.x, 0.15);
    this.smWriY = lerp(this.smWriY, wristTarget.y, 0.15);
    ...
    for(let i=0;i<this.N;i++){
      ...
      if(i === 0){
        ...
        this.dots[i].x=lerp(this.dots[i].x, tx2, 0.22);
        this.dots[i].y=lerp(this.dots[i].y, ty2, 0.22);
      }
    }
  }
