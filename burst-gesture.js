// ================================================================
//  A W A K E N I N G  —  gesture.js  (v8 + GRS hooks + BURST)
//
//  ── UNCHANGED from v8 ──────────────────────────────────────────
...
//
//  3. TWIRL-BURST SYSTEM:
//     - Detects when user turns their back (both shoulders invisible in Pose)
//     - window.burstState + window.burstColor exposed for gestureReactions.js
//     - Hero bursts apart and rematerialises ~4s later
//
//  All original logic untouched. New code is clearly marked NEW.
// ================================================================
...
// ─────────────────────────────────────────────────────────────────
//  PUPIL SYSTEM  (v8 — Doc3 exact values)
// ─────────────────────────────────────────────────────────────────
const PUPIL_MAX     = 9;
...
//  TWIRL-BURST SYSTEM
//
//  Trigger: user turns their back to the camera (a twirl / spin).
//  Detected via MediaPipe Pose: when BOTH shoulders become invisible
//  (shoulderSpread < 0.12) simultaneously for TWIRL_CONSEC frames, the
//  character bursts apart. It rematerialises 4 s later once the
//  user faces the camera again.
//
//  No velocity tracking, no threshold tuning — purely visibility-
//  based so it never fires from normal movement and never lags.
//
//  Globals exposed to gestureReactions.js (unchanged interface):
//    window.burstState  — "idle" | "burst" | "remat"
//    window.burstX/Y    — burst centre (hero position at trigger)
//    window.burstColor  — hero charId
//    window.burstRadius — hero body radius
// ─────────────────────────────────────────────────────────────────
const TWIRL_CONSEC   = 6;    // frames both shoulders invisible before firing (~100ms)
const BURST_HOLD     = 90;   // frames shattered before remat starts (~1.5s)
const BURST_REMAT    = 150;  // frames for particle reconverge + fade-in (~2.5s)
const BURST_COOLDOWN = 240;  // min frames before next burst (~4s)
...
let _twirlConsec = 0;   // consecutive frames both shoulders invisible
let _bTimer      = 0;   // frame counter within current burst state
let _bCooldown   = 0;   // cooldown frames remaining
...
function _updateBurst(){
  ...
  // ── Twirl detection (idle state only) ────────────────────────
  // Detects user showing their back by measuring shoulder-to-shoulder
  // horizontal distance. Facing camera: shoulders are far apart in X
  // (~0.3-0.5 of frame width). Showing back: they collapse toward the
  // same X position (distance < 0.10 of frame width).
  // Uses normalised MediaPipe coords (0-1) so it's resolution-independent.
  // No visibility field needed — works on all MediaPipe Pose versions.
  if(window.burstState === "idle"){
    const pd = window.mpPose;
    const lm = pd && pd.poseLandmarks;
    if(lm && lm.length >= 13){
      // lm coords are normalised 0-1. Shoulder spread in X.
      const lShoulderX = lm[11].x;
      const rShoulderX = lm[12].x;
      const shoulderSpread = Math.abs(lShoulderX - rShoulderX);
      // < 0.12 means shoulders are nearly on top of each other = back turned
  ...
  // ── Burst phase: character is shattered ──────────────────────
  ...
  // ── Remat phase: invisible until 75%, then fade in ───────────
  ...
  // ── Idle: hero always fully visible ──────────────────────────
...
function draw(){
  ...
  // ★ NEW — tick burst system each frame (only does work in companionMode)
  _updateBurst();
...
class BlobChar{
  ...
  draw(){
    ...
    if(this.mode==="awakening"&&companionMode){
      // ★ NEW — skip arms during burst (they'd float disconnected)
  ...
  _drawFaceWorld(){
    // ★ NEW — skip face during burst (body is gone)
    ...
    // ★ NEW — fade face alpha during remat
